...
This is a Twilio Proxy Administrative Application that facilitates the ability to manage participants and display participant interactions as part of a Proxy Session. Currently, the Twilio Console does not have a way to manage participants through the UI, so this app allows for a UI experience. The app provides very simple constructs for managing Services, Sessions, and Phone Numbers as well, but not to the depth that is provided in the Twilio Console UI.
