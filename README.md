...
This app is not a product sold by Twilio, and Twilio accepts no resposibility for your use of this app, nor is bound to support your use of this app or any modifications you make to this app for your own personal or business use.
...
```Step 7:```Drill into your [Programmable Chat Service](https://www.twilio.com/console/chat/dashboard) you setup in Step #1 and navigate to the "Webhooks" configuration. There is a "Post-Event Webhooks" section where you need to add the following as the Callback URL: https://webhooks.twilio.com/v1/Accounts/<ACSid>/Proxy/<KSSid>/Webhooks/ChatEvent/ProxyIdentifier/<PNSid>. Note that the "PNSid" really can be anything, it doesn't have to be the actual PNSid. The ACSid and KSSid, however, need to from your Account and Proxy Service respectively. You must check "OnMessageSent", "OnChannelUpdated", and "OnChannelDestroyed" for Callback Events.
